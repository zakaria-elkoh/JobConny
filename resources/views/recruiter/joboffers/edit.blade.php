...
{{-- @include('layouts.nav') --}} 
<form class="p-6 bg-[#333] max-w-screen-md mx-auto mt-28 mb-10 rounded-3xl" action="{{ route('recruiter.joboffers.update', ['joboffer' => $jobOffer->id]) }}" method="POST" enctype="multipart/form-data">

    @csrf
    @method('PUT') 

    <h2 class="text-3xl font-extrabold mb-5 text-center dark:text-white">Edit Job Offer:</h2>

    <div> 
        <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Job Offer Title:</label>
        <input type="text" name="title" id="title" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Title" value="{{ old('title', $jobOffer->title) }}">
        @error('title')
   ...
   <div>
    ...
    <div>
        ...
        <input type="text" name="city" id="city" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="City" value="{{ old('city', $jobOffer->city) }}">
        @error('city')
            <span class="text-red-400">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div>
        ...
        <select name="sector_id" id="sector_id" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            @foreach ($sectors as $sector) 
                <option value="{{ $sector->id }}" {{ $jobOffer->sector_id == $sector->id ? 'selected' : '' }}>
                    {{ $sector->title }}
                </option>
            @endforeach
        </select>
    ...
    <button type="submit" class="text-white block mx-auto bg-blue-700 focus:outline-none hover:bg-blue-800 focus:ring-4 font-medium rounded-full text-sm py-2.5 px-10 mt-5">Update Job Offer</button>
</form>